...
def obs2concavehull(obs,thresh=0.1,alpha=1):
    ...
    if number of points is only two then returns LineString
    if number of points is only one then returns Point
...
def match2obs_sub(df, mass, thresh=0.2,alpha=1):
    # get concave hull surrounding observations.
    chull, edge_points = obs2concavehull(mass,alpha=alpha)
    # get time of observations to nearest hour.
    obstime = roundtime(mass.time.values)
    temp = df[df.date == obstime]
    geometry = [sgeo.Point(xy) for xy in zip(temp['lon'],temp['lat'])]
    gdf = -1
    #gdf = gpd.GeoDataFrame(temp,geometry=geometry)
    # test whether the point is in the convex hull.
    # 'test' column is True if it is.
    #gdf['test'] = gdf['geometry'].within(chull)
    return gdf   
