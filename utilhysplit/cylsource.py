            ...
            for iht in range(0, 2):
                ...
                for ipolls in range(pollnum):
                    if iht == 0:
                        ...
                        rate_str = '0.0'
                    emitfile.write(datestr + ' ' + duration + '  ' + latstr + ' ' +
                                   lonstr + ' ' + htstr+' '+rate_str+' 0.0 ' + heat+'\n')
        if image == True:
            map2 = Basemap(projection="tmerc", lon_0=lonlist[0], lat_0=latlist[0], llcrnrlat=np.min(
                latlist)-1, urcrnrlat=np.max(latlist)+1, llcrnrlon=np.min(lonlist)-1, urcrnrlon=np.max(lonlist)+1)
            ...
            while h < len(height):
                ...
                for ip in range(pollnum):
                    if h == tt-2:
                        ...
                        rate_str = '0.0'
                    emitfile.write(datestr+' '+str(duration)+' '+latstr+' ' +
                                   lonstr+' '+hstr+' '+rate_str+' 0.0 0.0 \n')
                h += 1
        emitfile.close()
