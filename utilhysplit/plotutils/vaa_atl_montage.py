...
from utilhysplit.geohysplit import ensemble_polygons
...
class VAAMontageATL:
    ...
    def cmap(self,cmap):
        if isinstance(cmap,str):
           cmapstr = cmap
        # set based on prob threshold
        else:
            vaathresh = self.vaathresh
            if np.isclose(vaathresh,0.2,atol=0.01):
               cmap = 'Blues'
            elif np.isclose(vaathresh,2,atol=0.01):
               cmap = 'YlOrBr'
            elif np.isclose(vaathresh,5,atol=0.01):
               cmap = 'Reds'
            elif np.isclose(vaathresh,10,atol=0.01):
               cmap = 'RdPu'
            elif vaathresh < 0.2:
               cmap='Greys'
            elif vaathresh > 10:
    ...
    def rows(self):
        nnn = len(self.times)
        nrows = int(np.ceil(nnn/self.columns))
        # if all will be filled then add extra
        # row for text
        if nnn == nrows * self.columns:
           nrows += 1
        return nrows
    ...
    @property   
    def unit(self):
        rval = 'Percent above {} {} (maximum in column)'.format(self.vaathresh,self._unit)
        return rval
    ...
    def make_atl2(self):
        print('here')
        self._atl = self.cdump.sel(thresh=self.vaathresh)
    ...
    def levelgetter(self,zvals):
        return np.arange(0,105,5)
        #temp = np.where(zvals>0,zvals,np.nan)
        #minval = np.nanmin(temp)
        #print(minval)
        #minx = np.floor(minval)
        #if not np.isnan(minx):
        #   minx = int(minx)
        #else:
        #   minx = 1e-3
        #if minx < 5: dx = 5 
        #else: dx = minx
        #values = np.arange(0.01,100,5)
        #values[0] = minx
        #return values
    ...
    def plotpage(self):
        ...
        for iii, ax in enumerate(axlist.flatten()):
            ...
            if not skip:
                ...
                tpoly = top_poly.merge_with(bottom_poly,cmap='winter')
                handles, labels = tpoly.plot(ax=ax,pbuffer=0.15,legend=False, linewidth=1,transform=self.data_transform)

                p = 'None'
                zlist = []
                for val in  zip(handles,labels):
                    if val[1] != p: zlist.append(val)
                    p = val[1]
                handles, labels = zip(*zlist) 
                #print('HERE', labels)
            ...
            if row+1==self.rows-1: xlabel=True
            else: xlabel=False
...
def attrs2label(atthash,time,source=None,ens=None, thresh='Unknown', probthresh=0):
    ...
    label += '\n'
    label += 'Polygon shows area where at least {:0.1f}% ensemble members predict \n ash concentration above {:0.1f} mg/m$^3$ \n'.format(probthresh,thresh)
