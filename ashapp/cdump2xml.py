...
def generate_kmz(filenames, kmz_filename, compression_level):
    ...
    with zipfile.ZipFile(kmz_filename, "w", compresslevel=compression_level) as z:
        for f in filenames:
            if os.path.exists(f):
...
def AQI_folder():
    ...
    name.text = "EPA AIRNow Air Quality Index (AQI)"
    folder.append(name)
    viz = etree.SubElement(folder, "visibility")
    viz.text = "0"
...
def smoke_folder():
    folder = etree.Element("Folder")
    name = etree.Element("name")
    name.text = "NOAA NESDIS kml Smoke/Fire Data"
    folder.append(name)
    viz = etree.SubElement(folder, "visibility")
    viz.text = "0"
    description = etree.SubElement(folder, "description")
...
def weather_folder():
    folder = etree.Element("Folder")
    name = etree.Element("name")
    name.text = "NOAA NWS kml Weather Data"
    folder.append(name)
    viz = etree.SubElement(folder, "visibility")
    viz.text = "0"
    description = etree.SubElement(folder, "description")
...
def legend_namer(iii, jobid=""):
    ...
    iii : int
    """
    rval = "GELABEL_{:02d}_{}.png".format(iii, jobid)
    return rval
...
def generate_contours(cxra, levels, ht):
    qcs = plt.contour(
        cxra.longitude.values, cxra.latitude.values, cxra.values, levels=levels
    )
    iii = 0
...
class HysplitKml:
    ...
    def create_source_description(self):
        ...
        cstr += "\n"
        cstr += "Released between {} and {}".format(
            self.release_bottom, self.release_alt
        )
        cstr += "\n"
    ...
    def add_contours(
        self, tempra, iii=1, t1=datetime.datetime.now(), t2=datetime.datetime.now()
    ):
        levels = self.levels
        height = 100
