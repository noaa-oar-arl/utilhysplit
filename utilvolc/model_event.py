...
class ModelEventDF(EventDF):
    required = ["model_name", "file_name", "start_date", "end_date"]
...
class ModelForecast:
    ...
    def plot_mean_mass(self, vloc=None, extrasource=None, time_previous=3):
        ...
        dlist.sort()
        dstr = "{} to {}".format(
            dlist[0].strftime("%Y %m/%d %H:%M UTC"),
            dlist[-1].strftime("%m/%d %H:%M UTC"),
        )
    ...
    def plot_concentration(self, vloc=None, pagenum=0, time_previous=3):
        ...
        d0 = self.start_time - dt
        dstr = "{} to {}".format(
            d0.strftime("%Y %m/%d %H:%M UTC"),
            self.start_time.strftime("%m/%d %H:%M UTC"),
        )
    ...
    def plot_vaa_forecast(self, vloc, thresh=0.2, problev=50, plev=3):
        ...
        if plev == 0:
            vaa.plot(
                vloc=vloc, thresh=thresh, ppp="top", plotheight=True, plotmass=False
            )
            ...
            vaa.plot(
                vloc=vloc, thresh=thresh, ppp="top", plotheight=False, plotmass=True
            )
...
def pick_source(sss: str, drange: list) -> list:
    """ """
    new = []
    dlist = []
    for sname in sss:
