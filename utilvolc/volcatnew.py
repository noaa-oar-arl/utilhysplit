...
# 2023 Mar 01 AMC created for new data files. VolcatName is different. Also the _get_time function was modified.
#               
...
class VolcatName(FileNameInterface):
    ...
    def __init__(self, fname):
        ...
        self.image_end_date = None
       
        #self.event_date = None
        self.image_dtfmt = "s%Y%j%H%M"
        self.end_dtfmt = "e%Y%j%H%M"
        self.creation_dtfmt = "c%Y%j%H%M"
    ...
    def make_keylist(self):
        ...
        self.keylist.append('domain')
        self.keylist.append('event vid')
        self.keylist.append('volcant_version')
        self.keylist.append('WMO satellite id')
        self.keylist.append('image date')
        self.keylist.append('image end date')
        self.keylist.append('creation date')
    ...
    def parse(self, fname):
        fname = fname.replace('.nc','')
        print(fname)
        if "pc" in fname:
            self.pc_corrected = True
            fname = fname.replace('.pc','')
        vhash = {}
        temp = fname.split('-')
        vhash[self.keylist[0]] = temp[0]
        vhash[self.keylist[1]] = temp[1]
        
        temp2 = temp[-1].split('_')
        for iii, val in enumerate(temp2):
            vhash[self.keylist[iii+2]] = val
        self.vhash = vhash
        return vhash
    ...
    def get_dates(self):
        ...
        vhash = self.vhash
        self.image_date = datetime.datetime.strptime(vhash['image date'][0:-3], self.image_dtfmt)
        self.event_date = self.image_date
        self.image_end_date = datetime.datetime.strptime(vhash['image end date'][0:-3], self.end_dtfmt)
        self.date = datetime.datetime.strptime(vhash['creation date'][0:-3], self.creation_dtfmt)
        return
...
def _get_time(dset):
    ...
    # to do check that times are within a certain value of eachother.
    temp1 = dset.event_observation_time.values
    temp2 = temp1[0]
    if len(temp1) > 1:
       print('WARNING - multiple event observation times in file')
       print(temp1)
