...
class ModelBin:
    ...
    def __init__(
        ...
        """
        self.lon_neg = False # set True if need lon values -180,180.
    ...
    def makegrid(self, xindx, yindx):
        ...
        if self.lon_neg: lonlist = [fixlon(x) for x in lonlist]
...
def get_latlongrid(dset, xindx, yindx, neg=False):
    """
    INPUTS
    ...
    lonlist = [lon[x - 1] for x in xindx]
    latlist = [lat[x - 1] for x in yindx]
    if neg: lonlist = [fixlon(x) for x in lonlist]
    mgrid = np.meshgrid(lonlist, latlist)
    return mgrid


def getlatlon(dset, neg=False):
    """
    ...
    neg : boolean
          if True then longitude values take on range -180 to 180.
          if False then longitude values take on range 0 to 360.
    RETURNS
    ...
    if neg: lon = [fixlon(x) for x in lon]
    return lat, lon
